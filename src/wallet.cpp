...
/**
 * Fees smaller than this (in uCPC) are considered zero fee (for transaction creation)
...
void CWalletTx::RelayWalletTransaction(std::string strCommaPlusCoinnd)
{
    if (!IsCoinBase()) {
...
bool CWallet::MintableCoins()
{
    ...
    if (nBalance <= nReserveBalance){
        LogPrintf("[MintableCoins(): nBalance (%d) <= nReserveBalance (%d)", nBalance, nReserveBalance);
        return false;
    }
    ...
    for (const COutput& out : vCoins) {
        //check for min age
        if (GetAdjustedTime() - out.tx->GetTxTime() > nStakeMinAge){
            LogPrintf("[True] GetAdmustedTime() = %d, out.tx->GetTxTime() = %d, nStakeMinAge = %d", GetAdjustedTime(), out.tx->GetTxTime(), nStakeMinAge);
            return true;
        }
//        LogPrintf("[False]GetAdmustedTime() = %d, out.tx->GetTxTime() = %d, nStakeMinAge = %d", GetAdjustedTime(), out.tx->GetTxTime(), nStakeMinAge);
    }
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 CPC.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-CPC-address
                    CScript scriptChange;
                ...
                // send transaction fee to burn address
                if (nFeeRet >= 0){
                    CBitcoinAddress address(*(&BURN_ADDRESS));
                    CScript scriptPubKey = GetScriptForDestination(address.Get());
                    CTxOut txout(nFeeRet, scriptPubKey);
                    txNew.vout.push_back(txout);
                }
                int nIn = 0;
                BOOST_FOREACH (const PAIRTYPE(const CWalletTx*, unsigned int) & coin, setCoins)
 ...
 */
bool CWallet::CommitTransaction(CWalletTx& wtxNew, CReserveKey& reservekey, std::string strCommaPlusCoinnd)
{
    {
        LOCK2(cs_main, cs_wallet);
        ...
        wtxNew.RelayWalletTransaction(strCommaPlusCoinnd);
    }
    return true;
}
