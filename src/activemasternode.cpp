...
//
// Bootup the Masternode, look for a CPC collateral input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        //     if (service.GetPort() != 34520) {
        //         notCapableReason = strprintf("Invalid port: %u - only 34520 is supported on mainnet.", service.GetPort());
        //         LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //         return;
        //     }
        // } else if (service.GetPort() == 34520) {
        //     notCapableReason = strprintf("Invalid port: %u - 34520 is only supported on mainnet.", service.GetPort());
        //     LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //     return;
        // }
...
bool CActiveMasternode::SendMasternodePing(std::string& errorMessage)
{
    ...
    if (pmn != NULL) {
        ...
        mnp.Relay();
        //LogPrintf("CActiveMasternode::SendMasternodePing() - Return true\n");   // added by bomlinh
        return true;
    } else {
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    // if (Params().NetworkID() == CBaseChainParams::MAIN) {
    //     if (service.GetPort() != 34520) {
    //         errorMessage = strprintf("Invalid port %u for masternode %s - only 34520 is supported on mainnet.", service.GetPort(), strService);
    //         LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //         return false;
    //     }
    // } else if (service.GetPort() == 34520) {
    //     errorMessage = strprintf("Invalid port %u for masternode %s - 34520 is only supported on mainnet.", service.GetPort(), strService);
    //     LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //     return false;
    // }
