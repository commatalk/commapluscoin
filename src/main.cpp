...
#if defined(NDEBUG)
#error "CommaPlusCoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60*60; // linhho change from 60 to 60*60
int64_t nReserveBalance = 0;

/** Fees smaller than this (in uCPC) are considered zero fee (for relaying and mining)
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    if (!tx.IsCoinStake() && !tx.IsCoinBase() && tx.vout.size() >= 3){
        CTxOut txout = tx.vout.at(tx.vout.size() -1);
        if (!::IsStandard(txout.scriptPubKey, whichType)) {
            reason = "scriptpubkey";
            return false;
        }
        ...
        if (scriptPubKey.ToString() != txout.scriptPubKey.ToString()){
            reason = "scriptpubkey";
            return false;
        }
    }
...
bool CheckTransaction(const CTransaction& tx, CValidationState& state)
{
    ...
    if (!tx.IsCoinStake() && !tx.IsCoinBase() && tx.vout.size() >= 3){
        CTxOut txout = tx.vout[tx.vout.size()-1];
        CBitcoinAddress address(*(&BURN_ADDRESS));
        CScript scriptPubKey = GetScriptForDestination(address.Get());
        if (scriptPubKey.ToString() != txout.scriptPubKey.ToString()){
            return state.DoS(100, error("CheckTransaction() : txout.nValue invalid burn address"),
                REJECT_INVALID, "bad-txns-vout-invalid-burn-address");
        }
    }
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK()){
        nSubsidy = 14488000 * COIN;
    }

    // else if (nHeight <= Params().LAST_POW_BLOCK())
    //     nSubsidy = 10 * COIN; 
//    else if (nHeight < 6440)
//        nSubsidy = 10 * COIN;
...
//        nSubsidy = 15 * COIN;
//    else if (nHeight < 10828400)
//        nSubsidy = 5 * COIN;
     else if (nHeight < 10512044)
         nSubsidy = 200 * COIN;
    else
        nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    ret = 0; // 0% of masternode block reward
...
{
    RenameThread("commapluscoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // CPC: We use certain sporks during IBD, so check to see if they are
