...
CommaPlusCoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<commapluscoin_address\> \<monthly_payment_commapluscoin\> [use_ix(true|false)]
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<commapluscoin_address\> \<monthly_payment_commapluscoin\> \<collateral_hash\>
 ...
 * vote-alias         - Vote on a CommaPlusCoin initiative
 * vote               - Vote on a CommaPlusCoin initiative/budget
