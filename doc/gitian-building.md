...
*Setup instructions for a gitian build of CommaPlusCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CommaPlusCoin
...
- [Building CommaPlusCoin](#building-commapluscoin)
...
Clone the git repositories for commapluscoin and gitian and then checkout the commapluscoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the commapluscoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building CommaPlusCoin
----------------------

To build CommaPlusCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the commapluscoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/commapluscoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/commapluscoin/commapluscoin
...
For example:
```bash
URL=https://github.com/konez2k/commapluscoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit commapluscoin=${COMMIT} --url commapluscoin=${URL} ../commapluscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit commapluscoin=${COMMIT} --url commapluscoin=${URL} ../commapluscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit commapluscoin=${COMMIT} --url commapluscoin=${URL} ../commapluscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/commapluscoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/commapluscoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/commapluscoin-build.assert
```
...
[commapluscoin/gitian.sigs](https://github.com/commapluscoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
